env.info( "[CSG-1] dynamic_deck_population" )
--
...
function DYNDECK:Start()
    ...
    self:T("[CSG-1 DYNDECK] Add template menus to mission.")
    
    local menu_root

    --[[if CSG1.menu.root then
        menu_root = CSG1.menu.root
    else--]]
        -- add menu root
     ...
     for shipIndex, menuship in ipairs(DYNDECK.ship) do
        ...
        for templateIndex, template in ipairs(menuship.templates) do
            ...
            if template.group == "" then -- this is a complete template and should be placed in the Complete Templates submenu
                self:T("[CSG-1 DYNDECK] Add Full Template: " .. template.name)
                ...
                if not DYNDECK.menu[menuship.id].partial[template.group].templates[template.name] then -- check template entry isn't a duplicate
                    self:T("[CSG-1 DYNDECK] Add template submenu: " .. template.name)
                    ...
                    self:T("[CSG-1 DYNDECK] ERROR! Menu has already been added for template: " .. template.name)
                end
...
function DYNDECK:clearDeck(templateName, shipID, coalitionID, shipIndex, templateIndex)
    self:T("[CSG-1 DYNDECK] clearDeck called.")
...
function DYNDECK:applyTemplate(templateName, shipID, coalitionID, noClear, shipIndex, templateIndex)
    self:T("[CSG-1 DYNDECK] applyTemplate called.")
    ...
    if DYNDECK[templateName] then -- check called template exists!
        ...
        if not DYNDECK.ship[shipIndex].templates[templateIndex].active then
            ...
            self:T("[CSG-1 DYNDECK] TEMPLATE ALREADY ACTIVE!")
        end
 
    else -- if template does not exist
        _msg = "[CSG-1 DYNDECK] ERROR! REQUESTED SHIP TEMPLATE NOT FOUND: " .. templateName
        self:T(_msg)
...
-- default templates for SC and Forrestal. Additional templates should be placed in dynamicDeckTemplates.lua and loaded in ME after this file.
...
function DYNDECK.sc_flex4c234(shipID, templateName) 
	...
	local staticObj = {
		...
		["category"] = "Planes",
		["livery_id"] = "VFA-196 Main Battery 513 (1994-96)",
	...
	local staticObj = {
		...
		["type"] = "FA-18C_hornet", 			-- unit, category and livery of unit to place.
		["category"] = "Planes",
		["livery_id"] = "VFA-196 Main Battery 511 (1994-96)",
